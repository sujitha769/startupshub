import { founders } from './data'

const LinkedInIcon = () => (
  <svg className="w-4 h-4 fill-current" viewBox="0 0 24 24">
    <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
  </svg>
)

const Founders = () => (
  <section className="py-20 bg-slate-50">
    <div className="max-w-5xl mx-auto px-6">
      <div className="text-center mb-14">
        <span className="text-blue-600 font-semibold text-sm uppercase tracking-widest">The Team</span>
        <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mt-2 relative inline-block">
          Founders
          <div className="absolute -bottom-2 left-1/4 right-1/4 h-1 bg-yellow-400 rounded-full" />
        </h2>
      </div>

      <div className="grid md:grid-cols-2 gap-8">
        {founders.map((f) => (
          <div
            key={f.name}
            className="bg-white rounded-3xl p-8 shadow-sm border border-gray-100 hover:shadow-md transition-all flex gap-6 items-start"
          >
            <div
              className={[
                'w-16 h-16 rounded-2xl bg-gradient-to-br',
                f.color,
                'flex items-center justify-center text-white font-bold text-xl flex-shrink-0 shadow-lg'
              ].join(' ')}
            >
              {f.avatar}
            </div>
            <div className="flex-1">
              <h3 className="text-xl font-bold text-gray-900">{f.name}</h3>
              <span className="inline-block text-xs font-semibold text-blue-600 bg-blue-50 px-3 py-1 rounded-full mt-1 mb-3">
                {f.role}
              </span>
              <p className="text-gray-500 text-sm leading-relaxed">{f.bio}</p>
              <a
                href={f.linkedin}
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-2 mt-4 text-sm font-semibold text-blue-600 hover:text-blue-800 transition"
              >
                <LinkedInIcon />
                LinkedIn Profile
              </a>
            </div>
          </div>
        ))}
      </div>
    </div>
  </section>
)

export default Founders